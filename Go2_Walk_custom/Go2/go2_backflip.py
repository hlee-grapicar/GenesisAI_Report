...
def main():
    ...
    policy = torch.jit.load(f"./Go2/backflip/{args.exp_name}.pt")
    policy.to(device=gs.device)

    # reset the environment (parent reset returns a minimal obs)
    env.reset()
    # BackflipEnv overrides get_observations() to produce the full 60-dim obs
