...
class Go2Env:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False):
        ...
        self.actions_dof_idx = torch.argsort(self.motors_dof_idx) # motors_dof_idx 찾기 위해서

        # PD control parameters
        self.robot.set_dofs_kp([self.env_cfg["kp"]] * self.num_actions, self.motors_dof_idx) #모든 관절에 같은 값을 주기 위해 num_actions 곱함
        ...
        # Initial state
        _init_base_pos = torch.tensor(self.env_cfg["base_init_pos"], dtype=gs.tc_float, device=gs.device)
        if "visualization_offsets" in self.env_cfg:
            offsets = torch.tensor(self.env_cfg["visualization_offsets"], dtype=gs.tc_float, device=gs.device)
            self.init_base_pos = _init_base_pos.unsqueeze(0) + offsets
        else:
            self.init_base_pos = _init_base_pos.expand(self.num_envs, -1)

        _init_base_quat = torch.tensor(self.env_cfg["base_init_quat"], dtype=gs.tc_float, device=gs.device)
        self.init_base_quat = _init_base_quat.expand(self.num_envs, -1)

        self.inv_base_init_quat = inv_quat(_init_base_quat).expand(self.num_envs, -1)
        ...
        self.init_qpos = torch.concatenate((self.init_base_pos, self.init_base_quat, self.init_dof_pos), dim=-1) # 로봇 전체 위치 상태
        # unsqueeze global_gravity to support broadcasting
        self.init_projected_gravity = transform_by_quat(self.global_gravity.unsqueeze(0), self.inv_base_init_quat) #로봇 기준 중력 계산

        # initialize buffers
        # 시뮬레이션 중간 값을 저장해두기 위함
    ...
    def step(self, actions):
        self.actions = torch.clip(actions, -self.env_cfg["clip_actions"], self.env_cfg["clip_actions"]) #범위 안에 있도록 clip
        exec_actions = self.last_actions if self.simulate_action_latency else self.actions
        target_dof_pos = exec_actions * self.env_cfg["action_scale"] + self.default_dof_pos
        self.robot.control_dofs_position(target_dof_pos[:, self.actions_dof_idx], slice(6, 18)) #실제 움직이는 관절 위치 제어
        self.scene.step()
    ...
    def _reset_idx(self, envs_idx=None):
        # reset state
        if envs_idx is None or envs_idx.any():
            qpos_to_set = self.init_qpos if envs_idx is None else self.init_qpos[envs_idx]
            self.robot.set_qpos(qpos_to_set, envs_idx=envs_idx, zero_velocity=True, skip_forward=True)
